class Direction
  LEFT = /(1|2|3|4|5|6|7|8|9|0|one|two|three|four|five|six|seven|eight|nine|zero){1}/
  RIGHT = /(1|2|3|4|5|6|7|8|9|0|eno|owt|eerht|ruof|evif|xis|neves|thgie|enin|orez){1}/
  attr_reader :code

  def initialize(str)
    left = LEFT.match(str)[1]
    right = RIGHT.match(str.reverse)[1].reverse
    @code = [decode(left), decode(right)].join.to_i
  end

  def decode(digit)
    return digit if digit.to_i > 0
    %w[zero one two three four five six seven eight nine].find_index(digit).to_s
  end
end
...
puts codes.sum {|code| Direction.new(code).code }
